...
$columns_number = technum_get_theme_mod('portfolio_archive_columns_number');
$posts_per_page = technum_get_theme_mod('portfolio_archive_posts_per_page');
$paged          = get_query_var('paged') ? get_query_var('paged') : 1;

$query = new WP_Query( [
    'post_type'             => 'technum_portfolio',
                    ...
                    <div class="archive-listing-wrapper portfolio-listing-wrapper portfolio-grid-listing<?php echo ( isset($columns_number) && !empty($columns_number) ? ' columns-' . esc_attr($columns_number) : '' ); ?>">
                        <?php
                            while( $query->have_posts() ){
                                $query->the_post();
                                get_template_part('content', 'technum_portfolio');
                            };
                            wp_reset_postdata();
                        ?>
                    </div>
