...
$columns_number = technum_get_theme_mod('service_archive_columns_number');
$posts_per_page = technum_get_theme_mod('service_archive_posts_per_page');
$paged          = get_query_var('paged') ? get_query_var('paged') : 1;

$query = new WP_Query( [
    'post_type'             => 'technum_service',
                    ...
                    <div class="archive-listing-wrapper service-listing-wrapper service-grid-listing<?php echo ( isset($columns_number) && !empty($columns_number) ? ' columns-' . esc_attr($columns_number) : '' ); ?>">
                        <?php
                            while( $query->have_posts() ){
                                $query->the_post();
                                get_template_part('content', 'technum_service', array('excerpt_length' => technum_get_theme_mod('service_archive_excerpt_length')));
                            };
                            wp_reset_postdata();
                        ?>
                    </div>
