...
<div <?php post_class('standard-blog-item-wrapper'); ?>>
    <div class="blog-item">
        <?php
            if ( !empty(technum_post_date_output()) || !empty(technum_post_author_output()) ) {
                echo '<div class="post-meta-header">';
                    if ( !empty(technum_post_date_output()) ) {
                        echo technum_post_date_output(true);
                    }
                    if ( !empty(technum_post_author_output()) ) {
            ...
            if ( !empty(technum_post_tags_output()) ) {
                echo technum_post_tags_output(', ');
            }

            echo '<div class="post-more-button">';
                echo '<a href="' . esc_url(get_the_permalink()) . '">' . esc_html__('Read More', 'technum') . '</a>';
            echo '</div>';
        ?>
    </div>
</div>